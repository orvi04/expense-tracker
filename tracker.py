...
def save_menu():
    while True:
        ...
        if choice == "1":
            ...
            selection = input("Load save number: ")
            try:
                load_data(saves[int(selection) - 1])
            ...
            selection = input("Delete save number: ")
            try:
                index = int(selection) - 1
                if 0 <= index < len(saves):
                    file_to_delete = SAVES_DIR / f"{saves[index]}.json"
                    file_to_delete.unlink()
                    print(f"Deleted save '{saves[index]}'")
                else:
                    print("Invalid selection")
            except (ValueError, IndexError):
                print("Invalid selection")
